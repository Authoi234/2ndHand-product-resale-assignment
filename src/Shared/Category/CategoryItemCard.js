                ...
                <Tilt scale={1.05} glareEnable={true} glareMaxOpacity={0.8} glareColor="white" glarePosition="all" glareBorderRadius="20px" className="background-stripes parallax-effect" perspective={700}>
                    ...
                    <div className="card-body">
                        ...
                        <div className='text-start font-bold'>
                            <p className="text-lg text-white">Location: <span className="text-orange-500">{product.location.toString().slice(0, 5)}...</span></p>
                            <p className="text-white text-lg mt-3">Resale Price: <span className="text-orange-600">$ {product.resalePrice.toString().slice(0, 5)}...</span></p>
                            <p className="text-white text-lg mb-3">Original Price: <span className="text-orange-600">$ {product.originalPrice.toString().slice(0, 5)}...</span></p>
                            <p className="text-white text-lg mt-3">Years of use: <span className="text-orange-600">{product.yearsOfUse.toString().slice(0, 5)}...</span></p>
                            <p className="text-white text-lg mt-3">Year of purchase: <span className="text-orange-600">{product.yearOfPurchase.toString().slice(0, 5)}...</span></p>
                            <p className="text-white text-lg">Time when it got Posted: <span className="text-orange-600">{product.timeWhenItPosted.toString().slice(0, 5)}...</span></p>
                            <p className="text-white text-lg">Type: <span className="text-orange-600">{product.type}</span></p>
                            <p className="text-white text-lg">Condition Type: <span className="text-orange-600">{product.conditionType.toString().slice(0, 5)}...</span></p>
                            ...
                            <p className="text-white text-lg">{product.discription.slice(0,38)}.....</p>
                        </div>
                    </div>
                </Tilt>
                <div className='mb-5 -mt-1'>
                    <p className="text-lg font-semibold my-2 text-white">Sale Status: <span className="uppercase text-red-500">{product.status}</span></p>
                    <Link to={`/categoryItem/${product._id}`} className='btn btn-secondary text-lg'>Details Page | Purchase Page</Link>
